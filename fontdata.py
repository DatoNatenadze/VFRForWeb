...
class FontnetDataProvider(LabeledDataProvider):
    ...
    def make_batch(self, batch_ind):
        data, labels = make_batch_fontnet_random_crop_intensity(self.tp_test, self.tp_dataStore, self.tp_labelStore,
                                                                self.tp_batches[batch_ind], 128, self.squeeze_ratio,
    ...
    def get_next_batch(self):
        ...
        if self.feature_type != 0:
            data, labels = make_batch_fontnet_feature(self.tp_dataStore, self.tp_labelStore,
                                                      self.tp_batches[tp_batch_ind], 128, self.squeeze_ratio,
                                                      self.image_width, self.image_height, self.feature_type)
        else:
            if self.multiview:
                data, labels = tp_utils.make_multiview_batch_jstore(self.tp_dataStore, self.tp_labelStore,
                                                                    self.tp_batches[tp_batch_ind], self.tp_average)
            else:
...
def make_batch_fontnet_random_crop(dataStore, labelStore, batch, average, width, height):
    ...
    for count, i in enumerate(batch):
        ...
        img_dict = {'src': im, 'dst': np.reshape(data[count, :], [height, width]), 'A': A.astype(np.float32),
...
def image_5view(im_final, height, width, im_height, new_width):
    ...
    img_corner5 = im_final[(im_height - height) / 2:(im_height + height) / 2 + 1,
                  (new_width - width) / 2:(new_width + width) / 2 + 1]
...
def make_batch_fontnet_random_crop_intensity(test, dataStore, labelStore, batch, average, squeeze_ratio, width, height):
    ...
    for count, i in enumerate(batch):
        ...
        if test == False:
            ...
            img_dict = {'src': im_final, 'dst': np.reshape(data[count, :], [height, width]),
                        'param': np.array([A[0, 0], A[0, 1], A[1, 0], A[1, 1], b[0], b[1], fg, bg, theta, a]).astype(
                            np.float32)}
    ...
    if test == True:
        ...
        data = [np.transpose(data1), np.transpose(data2), np.transpose(data3), np.transpose(data4), np.transpose(data5),
                np.transpose(data01), np.transpose(data02), np.transpose(data03), np.transpose(data04),
                np.transpose(data05), np.transpose(data10), np.transpose(data20), np.transpose(data30),
                np.transpose(data40), np.transpose(data50)]
...
def generate_random_crop(width1, height1, width2, height2):
    ...
    while not success:
        ...
        for i in range(1, 10):
            ...
            if np.all(xy[0, :] > 1) and np.all(xy[0, :] < (width1 - 2)) and np.all(xy[1, :] > 1) and np.all(
                    xy[1, :] < (height1 - 2)):
                success = 1
                break
...
def generate_random_crop2(width1, height1, width2, height2):
    ...
    offset[0] = np.random.uniform(low=-(width1 - width2 * height1 / float(height2)) / 2.0,
                                  high=(width1 - width2 * height1 / float(height2)) / 2.0)
